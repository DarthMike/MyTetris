...
void GFXEffects::FadeInEntity(IND_Entity2d* entity,float fadetime, Vector3 &MovSpeed)
{
	...
	if(!_foundEffect(entity,FADEIN))
	{
		...
		EffectsParams* neweffect = new EffectsParams;
...
void GFXEffects::FadeOutEntity(IND_Entity2d* entity,float fadetime, Vector3 &MovSpeed)
{
	//Fadetime is in SECONDS, MovSpeed is in Pixels/second
	...
	if(!_foundEffect(entity,FADEOUT))
	{
		...
		neweffect->remainingtime = fadetime*1000;  //Time
		neweffect->timelimit = fadetime*1000;      //Time
		neweffect->param = (float)entity->GetTransparency(); //Transparency stored here (float value, not the byte of entity)
...
void GFXEffects::ShowTimedEntity(IND_Entity2d* entity, float time, Vector3 &MovSpeed)
{
	//time is in SECONDS
...
bool GFXEffects::IsEffectComplete(IND_Entity2d* entity)
{
...
void GFXEffects::StopEffect(IND_Entity2d* entity)
{
...
{
	mGFXLibraryMgr->Render->ClearViewPort(0,0,0);
}

//Gets an entity from the list (if exists)
GFXEffects::EffectsEntitiesListIterator GFXEffects::_getAffectedEntity(IND_Entity2d* entity)
{
...
bool GFXEffects::_foundEffect(IND_Entity2d *entity,EffectType type)
{
...
{
	//Get entity
	IND_Entity2d* entity = affected->entity;
...
//Clean dynamic memory contents
void GFXEffects::_cleanUp()
{
	EffectsEntitiesListIterator itr;
	//LOOP - Search for an entity
	for(itr = mAffectedEntities.begin(); itr != mAffectedEntities.end(); itr++)
