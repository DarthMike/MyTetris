...
public:
		...
		mTimePosition(0)
	{
		...
		_open(filepath);	
		SingletonLogMgr::Instance()->AddNewLine("OggStream","Ogg Stream Object created",DEBUG);
	}
	~OggStream()
	{
		//Release resources
		...
		ov_clear(&mOggStream);
		SingletonLogMgr::Instance()->AddNewLine("OggStream","Ogg Stream Object destructed",DEBUG);
	}
	//----- GET/SET FUNCTIONS -----
	const std::string &GetFilename() { return mFileName; }
	double GetProcessedTime() { return mTimePosition; }
	//----- OTHER FUNCTIONS -----
	void Stop();					// Stop playing the Ogg stream
